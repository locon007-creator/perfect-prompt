...
describe('global conditional survival and fragment completion', () => {
  ...
  it('turns grouped screen requirements into complete build-ready instructions', () => {
    ...
    expect(output).toMatch(/For each item, ask for Due date/i);
    expect(output).toMatch(/For each item, ask for Fixed or Variable/i);
    expect(output).toMatch(/For each item, ask for Expected amount only if Fixed/i);
    expect(output).toMatch(/Show Money received/i);
    expect(output).toMatch(/Show Bills due soon/i);
    expect(output).toMatch(/Show Money remaining/i);
    expect(output).not.toMatch(/^Show only what matters now:\s*(?:in Home)?\.?$/im);
  });
  ...
  it('does not emit low-signal sentence fragments as core features', () => {
    ...
    expect(output).not.toMatch(/^Been paid\.$/im);
    expect(output).not.toMatch(/^What remains\.$/im);
  });
});
