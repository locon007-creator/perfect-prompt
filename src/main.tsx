...
function App(){
 ...
 function navigate(next:Screen){setScreen(next);setMenuOpen(false);setBuildPickerOpen(false);setVisualPickerOpen(false);setConfirmClear(false)}
 function go(){try{const next=generate(idea,{buildType,visualStyle});setPrompt(next);setError('')}catch(e){setError(e instanceof Error?e.message:'Please add more detail.')}}
 ...
 return <main className="app-shell">{header}
  ...
  <section className="build-picker visual-picker selector-stack-item">
   <button className="build-picker-trigger visual-trigger" onClick={()=>{setVisualPickerOpen(v=>!v);setBuildPickerOpen(false)}} aria-expanded={visualPickerOpen}><span><small>How should it look?</small><strong>{visualProfile.label}</strong></span><span className={visualPickerOpen?'picker-arrow open':'picker-arrow'}>⌄</span></button>
