...
const cases: Array<{ type: BuildType; label: string; idea: string; medium: string; structure: string; completion: string }> = [
  ...
  { type: 'image', label: 'Image', idea: 'Create an image of a glass cabin at sunrise. Required features: mountain background.', medium: 'Image generation', structure: 'Only subject, composition, framing, lighting, or environment requirements explicitly required by the idea.', completion: 'composition' },
  { type: 'general', label: 'General Prompt', idea: 'Create a prompt for summarizing meeting notes. Required features: concise summary.', medium: 'General prompt', structure: 'Only output format, content, or response structure explicitly required by the idea.', completion: 'requested output' }
];
...
describe('build type output framing', () => {
  for (const item of cases) {
    it(`uses ${item.type} terminology without mutating Idea Lock`, () => {
      ...
      expect(output).toContain(`Project type: ${item.label}`);
  ...
  it('recognizes an explicitly web-based game platform', () => {
    const output = generate('Build a browser puzzle game for casual players. Required features: score tracking.', { buildType: 'game' });
    expect(output).toContain('Platform\n\nWeb');
  });
});
