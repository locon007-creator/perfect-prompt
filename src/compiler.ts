...
export function parseIdea(raw:string):IdeaLock {
 ...
 const rawRequired=list(requiredText);const excludedRequired=rawRequired.filter(x=>exclusionMatches.some(e=>x.toLowerCase().includes(e.toLowerCase())||e.toLowerCase().includes(x.toLowerCase())));if(excludedRequired.length)throw Error(`Required feature excluded: ${excludedRequired.join(', ')}`);const required=rawRequired;
...
export function compile(raw:string, options:GenerateOptions={}):Prompt {const lock=parseIdea(raw); const role=options.buildType?getSpecialistProfile(options.buildType).role:'You are a senior product designer and frontend engineer.'; return {role,mission:`Design ${lock.appName} to help ${lock.targetUser} ${lock.primaryJob}.`,lock,targetUser:lock.targetUser,platform:lock.platform,workflow:lock.workflow,screens:[...lock.screens],features:[...lock.requiredFeatures],states:[...lock.stateRules,...lock.persistenceRules],visual:lock.visualRequirements.join('; '),constraints:[...lock.constraints],doNotAdd:[...lock.explicitExclusions],completion:'Every locked requirement is implemented, the stated workflow is preserved, and no unrequested screens or features are added.'};}
